...
# Added here since the difference between TF1.0 and my env TF 2.7
...
class DeepQNetwork:
    ...
    def init_op(self):
        init_operator = tf.compat.v1.global_variables_initializer()
    ...
    def _build_net(self):
        ...
        with tf.compat.v1.variable_scope('eval_net'+str(self.index)):
            # c_names(collections_names) are the collections to store variables
            c_names, n_l1, n_l2, w_initializer, b_initializer = \
                ['eval_net_params', tf.compat.v1.GraphKeys.GLOBAL_VARIABLES], 16, 8, \
                tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)  # config of layers

            # first layer. collections is used later when assign to target net
            with tf.compat.v1.variable_scope('l1'+str(self.index)):
            ...
            with tf.compat.v1.variable_scope('l2'+str(self.index)):
                w2 = tf.compat.v1.get_variable('w2', [n_l1, n_l2], initializer=w_initializer, collections=c_names)
                b2 = tf.compat.v1.get_variable('b2', [1, n_l2], initializer=b_initializer, collections=c_names)
                l2 = tf.matmul(l1, w2) + b2

            # third layer. collections is used later when assign to target net
            with tf.compat.v1.variable_scope('l3'+str(self.index)):
                w3 = tf.compat.v1.get_variable('w3', [n_l2, self.n_actions], initializer=w_initializer, collections=c_names)
                b3 = tf.compat.v1.get_variable('b3', [1, self.n_actions], initializer=b_initializer, collections=c_names)
                self.q_eval = tf.matmul(l2, w3) + b3

        with tf.compat.v1.variable_scope('loss'+str(self.index)):
            self.loss = tf.reduce_mean(tf.compat.v1.squared_difference(self.q_target, self.q_eval))
        with tf.compat.v1.variable_scope('train'+str(self.index)):
        ...
        with tf.compat.v1.variable_scope('target_net'+str(self.index)):
            # c_names(collections_names) are the collections to store variables
